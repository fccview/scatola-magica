...
2. Click the **Add** (+) button in the bottom-right corner (or paste from clipboard anywhere on the screen and skip to step 7)
...
7. Select which files you want to download
...
1. Click the **Add** (+) button (or drop a file/paste from clipboard anywhere on the screen and skip to step 5)
...
By default, torrents download to your root folder (or user folder if you are not admin)
...
4. Choose options:
   - **Announced or Unannounced** - Unannounced torrents don't use trackers
   - **Trackers** - Only if creating an announced torrent
...
- No symlinks allowed (for security reasons if a folder is a symlink the torrent will not be created)
...
- **Start seeding immediately** - Click "Start Seeding" to begin sharing

Created torrents appear in the Torrents page. If a file has a torrent created for it, it'll show in the file list with a torrent icon.
...
**Trackers** (for announced torrents)

- List of tracker URLs to include in created torrents
- One per line
- Used only when creating announced torrents
